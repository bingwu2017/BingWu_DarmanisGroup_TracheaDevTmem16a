...
---
## This script is for analyzing 10x scRNA-seq data from E15, E16, P1, and P4 wild-type (WT) mice. Mice are WT is the sense that they don't have delections in Ano1.
...
##### First to load E15 data (nGene 1k, nUMI 5k already applied):
...
##### second to load seu_E16P1P4_wt.RData:

```{r}
load("seu_E16P1P4_wt.RData")
```

##### data has already been normalized and log transformed.

##### merge E15 wild-type data with E16P1P4 wild-type data:
...
##### Alternatively, seurat_E15E16P1P4_wt object can be created by merging 4 seurat objects (saved in E15_Oct10_10X_Trachea.RData, E16_Dec7v3_Trachea.RData, seu_P1_Dec11_mm10.1.2.0.RData, and P4_10X_mm10_1.2.0.RData) and taking the wild-type subset. 
...
##### k.param=15 to have better distinction for rare cell types:
...
allWT_marker.res.1.4<-FindAllMarkers(object = seurat_E15E16P1P4_wt, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
write.table(allWT_marker.res.1.4,"allWT_marker.res.1.4.txt",sep="\t")

```


```{r}
allWT_marker.res.1.4 %>% group_by(cluster) %>% top_n(10, avg_logFC)
```
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("doublet_score"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'
))

```

##### potential red blood cells:
...
##### some cells potentially come from the thyroid:
...
##### use nonShh subset clustering information (seurat object: E15E16P1P4_wt_nonShh resolution=1.6) to further annotate those non-epithelial cells:

```{r}
seurat_E15E16P1P4_wt<-AddMetaData(object = seurat_E15E16P1P4_wt, metadata = wt_nonShh_cellType1.6, col.name = "specific_type")
```
```{r}
table(wt_nonShh_cellType1.6)
```
...
```{r}
table(seurat_E15E16P1P4_wt@meta.data$cell_type,seurat_E15E16P1P4_wt@meta.data$res.1.4)
```

```{r}
table(seurat_E15E16P1P4_wt@meta.data$specific_type,seurat_E15E16P1P4_wt@meta.data$res.1.4)
```
...
```{r}
table(seurat_E15E16P1P4_wt@meta.data$specific_type,seurat_E15E16P1P4_wt@meta.data$age)
```
...
allWT_marker.specificType<-FindAllMarkers(object = seurat_E15E16P1P4_wt, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
write.table(allWT_marker.specificType,"allWT_marker.specificType.txt",sep="\t")

```

```{r}
allWT_marker.specificType %>% group_by(cluster) %>% top_n(10, avg_logFC)
```

```{r,fig.width=10,fig.height=5}

TSNEPlot(object = seurat_E15E16P1P4_wt, do.label = F,pt.size = 0.2,group.by="specific_type")+scale_color_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Ccl20","Ccl25","Ccr6","Ccr1","Ccr7","Ccr9","Cxcl13","Cxcr5","Ccr4"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")

```

##### IgA and IgM related stuff: Igha and Ighm are not in the reference

```{r,fig.height=12,fig.width=10}
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Fcamr","Pigr","Aicda"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")
...
DoHeatmap(object = average_wt_specific_Annotation, genes.use = c("Mki67","Epcam","Trp63","Krt5","Muc5b","Tff2","Spdef","Creb3l1","Ltf","Foxj1","Cdhr3","Twist2","Wnt2","Ly6a","Pi16","Thy1","Cd34","Acan","Col2a1","Sox9","Acta2","Myh11","Rgs5","Notch3","Pecam1","Lyve1","Fcer1g","Cd3g","C1qa","Cx3cr1","Plp1","Mpz","Tubb3","Snap25","Ascl1","Chga"), 
    slim.col.label = TRUE, group.label.rot = TRUE,use.scaled = T,group.cex = 30,cex.row=20,cells.use = average_wt_specific_Annotation@cell.names[!(average_wt_specific_Annotation@cell.names %in% c("Thyroid","Doublet","RBC"))],group.order = c("CyclingEpithelial","Basal","Transitional","Secretory","Ciliated","CiliaSecretory","Fibroblast","CyclingFibroblast","MesenchymalProgenitor","Chondrocyte","Muscle","VSMC/pericyte","LymphaticEndothelial","VascularEndothelial","Immune_1","Immune_2","SchwannCell","Neuron/NEC"))
```
...
seurat_E15E16P1P4_wt@ident = factor(seurat_E15E16P1P4_wt@ident,levels(seurat_E15E16P1P4_wt@ident)[c(5,1,19,17,4,3,14,16,20,11,9,10,13,21,12,2,8,6,18,15,7)])
DotPlot(object = seurat_E15E16P1P4_wt, cols.use = c("forestgreen","magenta3"),genes.plot = c("Mki67","Epcam","Trp63","Krt5","Muc5b","Tff2","Spdef","Creb3l1","Ltf","Foxj1","Cdhr3","Tubb3","Snap25","Ascl1","Chga","Plp1","Mpz","Pecam1","Lyve1","Fcer1g","Cd3g","C1qa","Cx3cr1","Acta2","Myh11","Rgs5","Notch3","Wnt2","Ly6a","Pi16","Thy1","Cd34","Acan","Col2a1","Sox9","Twist2"),group.by = "ident", x.lab.rot = T,plot.legend = T,col.max = 2,col.min = -2)
```
...
ggplot(df_wt[df_wt$res.1.4 %in% c(11,8,3,9,1,29,30,2,7,20,27,28),],aes(specific_type,Cftr))+facet_grid(.~age)+geom_dotplot(binaxis="y",aes(color=age,fill=age),binwidth=0.02,stackdir="center",position=position_dodge(0.8), dotsize=0.018)+ theme(axis.text.x = element_text(angle = 45,hjust=1))
```
...
```
```{r, fig.height=5, fig.width=12}
ggplot(df_wt[df_wt$res.1.4 %in% c(11,8,3,9,1,29,30,2,7,20,27,28),],aes(specific_type,Ano1))+facet_grid(.~age)+geom_dotplot(binaxis="y",aes(color=age,fill=age),binwidth=0.02,stackdir="center",position=position_dodge(0.8), dotsize=0.018)+ theme(axis.text.x = element_text(angle = 45,hjust=1))
```
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Sox10","Mpz","Shh","Phox2a","Phox2b"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'))
```

```{r,fig.height=4,fig.width=20}
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Sox10"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'))
```
```{r,fig.height=4,fig.width=20}
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Mpz"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'))
```
```{r,fig.height=4,fig.width=20}
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Shh"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'))
```
```{r,fig.height=4,fig.width=20}
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Phox2a"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'))
```
```{r,fig.height=4,fig.width=20}
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Phox2b"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type")+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080'))
```
...
```{r,fig.height=5,fig.width=10}
seurat_E15E16P1P4_wt<-SetAllIdent(seurat_E15E16P1P4_wt,id="specific_type")
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("PCD_score"), nCol = 1,x.lab.rot = T,point.size.use = 0.3,use.raw=F,group.by="specific_type",do.sort = F,ident.include = c("CyclingEpithelial","Basal","Transitional","Secretory","Ciliated","CiliaSecretory","Fibroblast","CyclingFibroblast","MesenchymalProgenitor","Chondrocyte","Muscle","VSMC/pericyte","LymphaticEndothelial","VascularEndothelial","Immune_1","Immune_2","SchwannCell","Neuron/NEC"))+scale_fill_manual(values=c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#000000', '#800000', '#aaffc3', '#ffd8b1', '#000075', '#808080'
))
...
```{r}
write.table(E15E16P1P4_wt_res14_29over1,"E15E16P1P4_wt_res14_29over1.txt",sep="\t")

```


```{r}
E15E16P1P4_wt_res14_1over29<-FindMarkers(seurat_E15E16P1P4_wt,ident.1=c(1),ident.2 = c(29),only.pos = TRUE)
E15E16P1P4_wt_res14_1over29
```


```{r}
write.table(E15E16P1P4_wt_res14_1over29,"E15E16P1P4_wt_res14_1over29.txt",sep="\t")
...
```
```{r}
df_3_1_29_wt<-FetchData(seurat_E15E16P1P4_wt,c("Trp63","Basp1","Nnat","Mmp11","Cldn6","Dbn1","Mdk","Id2","Id3","Wnt7b","Cldn10","Ccl20","Fmo2","Nupr1","Wnt4","Krt5","Krt15","Eln","Aqp3","Aqp4","Pglyrp1","Adm","Aqp5","Rps26-ps1","Rpl6l","Rpl9-ps6","Rpl13-ps3","Synpr","Tgm2","Trib1","age","res.1.4"))
...
library(gplots)
library(viridis)
heatmap.2(t(as.matrix(df_3_1_29_wt[df_3_1_29_wt$res.1.4 %in% c(3,1,29),1:31])),col=plasma(500), scale="row",Colv = NA, Rowv = NA,ColSideColors=c("#006d2c","#2ca25f","#66c2a4","#99d8c9")[as.numeric(as.factor(age))],labCol =NA,density.info="none",trace="none",dendrogram='none',srtCol=45,cexCol = 1,colsep=c(1018,2342),breaks=seq(-3,3,length.out=501))
...
library(gplots)
heatmap.2(t(as.matrix(df_3_1_29_wt[df_3_1_29_wt$res.1.4 %in% c(3,1,29),c(1:2,4:10,11:17,19:25,28:30)])),col=cividis(500), scale="row",Colv = NA, Rowv = NA,ColSideColors=c("#006d2c","#2ca25f","#66c2a4","#99d8c9")[as.numeric(as.factor(age))],labCol =NA,density.info="none",trace="none",dendrogram='none',srtCol=45,cexCol = 1,colsep=c(1018,2342),breaks=seq(-3,3,length.out=501))
par(lend = 1)           # square line ends for the color legend
legend("left",      # location of the legend on the heatmap plot
    legend = c("E15", "E16", "P1","P4"), # category labels
    col = c("#006d2c","#2ca25f","#66c2a4","#99d8c9"),  # color key
...
```{r}
df_20_27_wt<-FetchData(seurat_E15E16P1P4_wt,c("Foxj1","Prr18","Fgl1","Calb2","Stil","Foxn4","Shisa8","Acot7","Lrrc23","Ccdc67","Cep152","Plk4","Ccna1","Ccno","Cdc20b","Cdc20","Hyls1","Mcidas","Smim24","Ifi44","Aoc1","Wnt11","Irf7","Kcnk12","Pdxk","Vpreb3","Calml4","Sntn","Stmnd1","Ldlrad1","Cdhr4","Cdhr3","Slc23a2","Ly6a","Ly6c1","Adam8","Ifitm1","age","res.1.4"))
...
```{r, fig.width=8, fig.height=5}
library(gplots)
heatmap.2(t(as.matrix(df_20_27_wt[df_20_27_wt$res.1.4 %in% c(20,27),1:39])),col=plasma(500), scale="row",Colv = NA, Rowv = NA,ColSideColors=c("#006d2c","#2ca25f","#66c2a4","#99d8c9")[as.numeric(as.factor(age))],labCol =NA,density.info="none",trace="none",dendrogram='none',srtCol=45,cexCol = 1,colsep=c(362),breaks=seq(-3,3,length.out=501))
par(lend = 1)           # square line ends for the color legend
legend("left",      # location of the legend on the heatmap plot
    legend = c("E15", "E16", "P1","P4"), # category labels
    col = c("#006d2c","#2ca25f","#66c2a4","#99d8c9"),  # color key
...
```{r, fig.width=8, fig.height=5}
library(gplots)
heatmap.2(t(as.matrix(df_20_27_wt[df_20_27_wt$res.1.4 %in% c(20,27),c(1:2,5:7,9:14,16:19,21:22,24,28:33,36,37)])),col=cividis(500), scale="row",Colv = NA, Rowv = NA,ColSideColors=c("#006d2c","#2ca25f","#66c2a4","#99d8c9")[as.numeric(as.factor(age))],labCol =NA,density.info="none",trace="none",dendrogram='none',srtCol=45,cexCol = 1,colsep=c(362),breaks=seq(-3,3,length.out=501))
par(lend = 1)           # square line ends for the color legend
legend("left",      # location of the legend on the heatmap plot
    legend = c("E15", "E16", "P1","P4"), # category labels
    col = c("#006d2c","#2ca25f","#66c2a4","#99d8c9"),  # color key
...
```{r}
write.table(E15E16P1P4_wt_res14_2over7,"E15E16P1P4_wt_res14_2over7.txt",sep="\t")
...
```{r}
write.table(E15E16P1P4_wt_res14_7over2,"E15E16P1P4_wt_res14_7over2.txt",sep="\t")
...
E15E16P1P4_wt_res14_c2and7diffALL<-FindMarkers(seurat_E15E16P1P4_wt,ident.1=c(2),ident.2 = c(7),only.pos = F,logfc.threshold=0,min.pct=0,test.use="wilcox")
E15E16P1P4_wt_res14_c2and7diffALL
```

```{r}
write.table(E15E16P1P4_wt_res14_c2and7diffALL,"E15E16P1P4_wt_res14_c2and7diffALL.txt",sep="\t")
...
E15E16P1P4_wt_res14_c20and27diffALL<-FindMarkers(seurat_E15E16P1P4_wt,ident.1=c(20),ident.2 = c(27),only.pos = F,logfc.threshold=0,min.pct=0,test.use="wilcox")
E15E16P1P4_wt_res14_c20and27diffALL
```

```{r}
write.table(E15E16P1P4_wt_res14_c20and27diffALL,"E15E16P1P4_wt_res14_c20and27diffALL.txt",sep="\t")

```

```{r,fig.height=8,fig.width=40}

DoHeatmap(object = seurat_E15E16P1P4_wt, genes.use = c("Spdef","Creb3l1","Egln3","Serpine1","Egr2","B4galt5","Tspan33","Cited1","Dnajb9","Klk10","Klk11","Klk13","Foxa3","Upk3bl","Socs3","Gadd45g","Muc5ac","Muc5b","Muc1","Ltf","Epas1","Spink1","Sec14l3","Pigr","Crip1","Gabrp","Adh7","Muc4","Gp2","Tff2","Sftpd","Scgb1a1"), 
...
```
```{r}
df_2_7_wt<-FetchData(seurat_E15E16P1P4_wt,c("Spdef","Creb3l1","Egln3","Serpine1","Egr2","Cited1","Dnajb9","Klk11","Klk13","Foxa3","Gadd45g","Muc5ac","Muc5b","Gp2","Muc1","Muc4","Ltf","Epas1","Sec14l3","Pigr","Crip1","Gabrp","Adh7","Tff2","Sftpd","Scgb1a1","age","res.1.4"))
...
library(gplots)
heatmap.2(t(as.matrix(df_2_7_wt[df_2_7_wt$res.1.4 %in% c(2,7),1:26])),col=cividis(500), scale="row",Colv = NA, Rowv = NA,ColSideColors=c("#2ca25f","#66c2a4","#99d8c9")[as.numeric(as.factor(age))],labCol =NA,density.info="none",trace="none",dendrogram='none',srtCol=45,cexCol = 1,colsep=c(1094),breaks=seq(-3,3,length.out=501))
par(lend = 1)           # square line ends for the color legend
legend("left",      # location of the legend on the heatmap plot
    legend = c( "E16", "P1","P4"), # category labels
    col = c("#2ca25f","#66c2a4","#99d8c9"),  # color key
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Foxj1"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Mcidas"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Cdhr3"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Creb3l1"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Spdef"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Gp2"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
...
VlnPlot(object = seurat_E15E16P1P4_wt, features.plot = c("Prr18"), nCol = 3,x.lab.rot = T,point.size.use = 0.1,use.raw=F,group.by="specific_type",ident.include =c(2,7,20,27,28) )+scale_fill_manual(values=c('#EC906B', '#8E9FC5', '#AFD467'
))
